...
An alternative way to estimating the parameters of interest than the SEM approach described above is an item-response approach which assumes that each tasks can be described by a Rasch (1PL) model, meaning that all items (trials) measure the same latent ability with the same discrimination but potentially varying difficulty. The benefit of this approach is that it makes full use of whatever data there is available for a given task, time point and subject; it does not e.g. require differentially collapsing scores for test-halves. The downside is that the resulting estimates show greater uncertainty compared to the SEM estimates. Furthermore, to reap the benefits of this approach, it requires imposing additional constraints which we outline below. Most importantly, however, we think that the parameters estimated via the SEM approach are conceptually better aligned with the questions we are asking, in particular when it comes to reliability: as explained below, reliability in the Rasch model does not make a claim about the measurement instrument but about how well individuals can be separated from one another via the latent ability estimates that are obtained via the instrument. Nonetheless, we see the appeal of this approach (in particular the ease of implementation via open source software packages such as `brms`) and we therefore want to present it here as an alternative. Overall, the two approaches yield very similar results. [Jana: please expand/adjust]   
...
Parameterized as such, this model estimates an easiness parameter for each item (i.e. trial) as well as an easiness parameter for each time point, which is equivalent to a latent sate and which allows for assessing change over time. However, in contrast to the SEM approach, the absolute values of this parameter are not comparable across tasks. Additionally, it is possible to compute the reliability of the latent state estimates for each time point [see @adams2005reliability; @moosbrugger2007testtheorie]. As noted above, this conception of reliability is different from the SEM approach. It quantifies how reliably individuals can be separated from one another based on the trait estimates; in the SEM approach, reliability is a property of the measurement instrument (test-half in our case). Thus, these values cannot be compared directly across methods. 

Finally, we estimate the correlations between latent states as an indicator of stability of individual differences across time points. Here the interpretation is the same as in the SEM approach. A problem for estimating these correlations is that the default LKJ prior built in to `brms`. Regardless of which value is chosen, it has a very strong influence on the results for sample sizes as small as the one in our study [Jana: do you have a reference for that?]. To get around this problem, we fit pairwise models for all combinations of time points to estimate the correlations because in such models the prior over the covariance matrix is uniform.
...
```{r lseplotrasch, fig.height = 5, fig.width=10, fig.cap = "Latent state means and trait reliability estimates with 95\\% CrI for each time point estimated assuming a Rasch model for each task and time point.", out.width="100%"}
plot_grid(plsm_rasch, pls_rel, ncol = 1,labels = c("A", "B"), align = "v",axis = "l", rel_heights = c(1,1))
```


Supplementary Figure \@ref(fig:lseplotrasch) shows the latent state means and the trait reliabilities. The temporal dynamics in the latent trait estimates are comparable to the SEM results, even though the Rasch-based estimates seem more variable. The reliability estimates are higher compared to the SEM approach, however, please note that they have a different interpretation because they refer to the separability of individuals (trait reliability) and not the reliability of the measurement instrument. 
...
```{r lscorplotrasch, fig.height=4, fig.cap = "Correlations between latent states estimated assuming a Rasch model for each task and time point. Correlations have been estimated via pairwise models for all possible time points.", out.width="100%"}
plsc_rasch
```
...
From this model, we computed the consistency, that is, the proportion of variance that is explained by this trait variable. This value is comparable across the two approaches and we therefore visualize them jointly. In addition, we compute the reliability of the trait (person separability). As noted above, this values is not directly comparable to the reliability estimates sensu SEM.  

Supplementary Figure \@ref(fig:lstmplotcomp) shows the consistency and reliability estimates for the LST model. In comparison to the SEM approach, consistency values were comparable but on average slightly lower. The trait reliability values are, on the other hand, very high.   
...
rel_sum_constr%>%mutate(
  ...
  scale_color_colorblind(name = "")+
  facet_grid(~task)+
  theme(legend.direction = "vertical", legend.position = c(0.07, 0.25), legend.box = "horizontal", legend.background = element_blank()
    )
```

```{r lstmplotcomp, fig.height=3,fig.width=10, fig.cap = "Consistency estimates for the different tasks by estimation methods; reliability of trait estimates based on Rasch model.", out.width="100%"}
con_comp
```
