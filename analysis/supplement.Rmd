...
Example videos for each task can be found in the associated online repository in [*masked for peer revie*w]. 
...
In addition to analyzing the correlations between trait estimates, we analysed the correlations between learning rates. This analysis addresses the potential concern that the correlations between the trait estimates for the different tasks reflect differences in associative learning abilities, in that some individuals consistently learn faster than others which leads to individual differences in performance.

To assess correlations between learning rates, for each individual, we fit a model to the data from each task, predicting performance by time point (`brms` notation: `sum|trials(n) ~ time_point`; with `sum` being the number of correct responses per time point, `n` being the number of trials per time point and `time_point` enumerating the time point (1 to 10)).
