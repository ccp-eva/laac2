...
data_task <- read.csv("../data/laac2_data_task.csv")%>%
  ...
  mutate(task = factor(task, levels = c("Population-to-sample","Probabilistic-reasoning","Communicative-cues", "Ignore-visible-food", "Self-ordered-search","Attention-following" )))
...
We then move on to our [analytical framework](#analytical-framework). First we describe the [Structural Equation models](#structural-equation-modeling) that were used to investigate mean change, stability and reliability of cognitive performance. Next, we describe the [Projection Predictive Inference](#projection-predictive-inference) analysis which we used to test the importance of the predictor variables.
...
All data sets, model outputs and analysis code are available in the associated online repository https://github.com/ccp-eva/laac2/. For a permanently archived version of the repository see the Zenodo link in the paper. The repository also contains additional documentation of the experimental procedures and example videos for the different tasks.
...
A second coder unfamiliar to the purpose of the study coded 20% of all time points (2 out of 10) for all tasks. Inter-rater reliability was excellent (lowest proportion of agreement = `r round(min(reli_summary$match),2)` for population-to-sample, lowest $\kappa$ = `r round(min(reli_summary$kappa),2)` for ignore-visible-food).

## Data collection {#data-collection}

One time point meant running all tasks with all participants. Within each time point, the tasks were organized in three sessions (see Supplementary Figure \@ref(fig:setup)). Session 1 included the population-to-sample and probabilistic-reasoning tasks, session 2 the communicative-cues and ignore-visible-food tasks and session 3 the self-ordered-search and attention-following tasks.
...
To capture change in performance that was due to repeatedly doing the same tasks, we included a variable in the model enumerating the time point at which the data was collected. Variable name in model: `Time point`.
...
psex <- data_trial%>%
  ...
  scale_y_continuous(limits = c(0, 15), breaks = c(1,5, 10, 15), labels = c(1,5, 10, 15))+
  labs(x="", y = "Frequency")+
...
pdist <- glife%>%
  filter(measure == "Disturbance")%>%
  ggplot(aes(x = factor(time_point), y = value, col = group))+
  geom_point()+
  geom_line(aes(group = group),alpha = .75)+
  labs(x = "", y = "Prop. days with disturbance")+
  scale_color_ptol(name = "Group", guide = "none")+
  theme_minimal()+
  theme(panel.border = element_rect(colour = "black", fill=NA, size = .5))

pevent <- glife%>%
  filter(measure == "Life event")%>%
  ggplot(aes(x = factor(time_point), y = value, col = group))+
  geom_point()+
  geom_line(aes(group = group),alpha = .75)+
  labs(x = "", y = "Prop. days with life event")+
  scale_color_ptol(name = "Group", guide = "none")+
  theme_minimal()+
  theme(panel.border = element_rect(colour = "black", fill=NA, size = .5))

ptime <- glife%>%
  filter(measure == "Time spent outdoors")%>%
  ggplot(aes(x = factor(time_point), y = value, col = group))+
  geom_point()+
  geom_line(aes(group = group),alpha = .75)+
  labs(x = "Time point", y = "Average hours spent outdoors")+
  scale_color_ptol(name = "Group")+
  theme_minimal()+
  theme(panel.border = element_rect(colour = "black", fill=NA, size = .5), legend.position = "bottom")


plife <- plot_grid(pdist,pevent,ptime, ncol = 1, rel_heights = c(1,1,1.2), align = "v")
```

```{r glife, out.width="100%", fig.height = 8, fig.width=8, fig.cap = "Variation in group life related measures across groups and time points."}
plife
```
...
pobs <- gtest%>%
  filter(measure == "Observer present")%>%
  ggplot(aes(x = factor(time_point), y = value, col = group))+
  geom_point()+
  geom_line(aes(group = group),alpha = .75)+
  labs(x = "", y = "Prop. sessions with \nobserver present")+
  scale_color_ptol(name = "Group", guide = "none")+
  theme_minimal()+
...
ppartd <- gtest%>%
  filter(measure == "Study participation\n (day)")%>%
  ggplot(aes(x = factor(time_point), y = value, col = group))+
  geom_point()+
  geom_line(aes(group = group),alpha = .75)+
  labs(x = "", y = "Prop. sessions with \nstudy participantion in different \nstudy on the same day")+
  scale_color_ptol(name = "Group", guide = "none")+
  theme_minimal()+
  theme(strip.text = element_text(size = 8), panel.border = element_rect(colour = "black", fill=NA, size = .5))
...
We used latent state-trait models to separate traits (stable over time) from state residual variables (time varying). In the present context, one can think of a trait as a stable psychological ability (e.g. ability to make causal inferences) and state residuals as time-specific deviations of the time-specific true score from these traits due to variable psychological conditions (e.g. variations in performance due to being attentive or inattentive). Variation in performance on a given time point can then be partitioned into variance due to the trait, variance due to the situation or individual-situation interactions, and measurement error. Because the latent variables are estimated on multiple indicators (here: test halves), they are assumed to be measurement-error free [@steyer1992states; @steyer2015theory; @geiser2020longitudinal]. Next we describe the model construction process in more detail.

At each time point, we observed 12 identical trials per individual per task. We modeled sum scores of the repeated trials, given that each trial per task was an identical repetition of the same task. To separate reliable from unreliable variance components and obtain reliability estimates of the resulting sum score variables, we built two sum score variables per task per time point. That is, for each task, two parallel test halves were built, corresponding to performance sum scores of half of the trials of the same time point per task. Trials were alternately assigned to the first and the second test half. For all tasks except self-ordered-search this procedure resulted in two test halves with 7 possible values (0 to 6 correctly solved trials). For self-ordered-search, because one could search in the same location twice on each trial, test halves could maximally assume 13 possible values (0 to 12 errors). 
...
For each task, we constructed two models. We started with a latent state (LS) model, which estimates a latent state for each time point based on the two test halves. Mean change in task-level performance can be assessed by comparing latent state means across time points. Stability of individual differences can be assessed by correlating latent state variables across different time points.

Second, we fit a latent state-trait (LST) model. In LST models, true inter-individual differences are decomposed into a latent trait variable and time-specific deviations of the true score from the latent trait (state residual variable). Usually, LST models assume stable latent trait variables across time (no trait change). After inspecting the data, we relaxed this assumption to account for the mean change in performance over time. Thus, we fitted LST models with varying intercept parameters over time. These intercept parameters for the time-specific true scores correspond to the average performance levels. Note that there is one intercept per time point, while the threshold parameters are set equal across test halves and time points. Thereby, we allowed for mean change in performance across time, which does not follow a pre-specified functional form. This model allowed us to partition the true variance in performance into stable (trait) and variable (state residual) components.
...
To allow for mean change across time within the LST model, we introduced an additional intercept parameter, with 

```{=tex}
\begin{equation}
Y^*_{it}= \alpha_t + T + \zeta_t + \epsilon_{it}
\end{equation}
```

where $\alpha_1=0$ and $\mathbf{E}(T)=\mathbf{E}(\zeta_t)=\mathbf{E}(\epsilon{it})=0$, and threshold parameters are set invariant across time and test halves.

As noted above, we assume strict factorial (measurement) invariance. Additionally, for reasons of parsimony, we assume that the variances of the state residual variables are invariant across time. As a consequence, the specified LST model corresponds to a multilevel model with a latent trait factor at the between-level (person-level) and a latent state residual factor at the within-level (time-specific) level.
...
Correlations between the the different tasks were estimated for the latent traits, that is, with respect to measurement-error free time-stable individual abilities. As the estimation of models combining several of the tasks runs into estimation problems due to item sparseness in the large number of categorical indicators (test halves times time points times tasks), we chose to estimate the correlations in a two-step procedure. To this end, we ran the LST models with varying means, as described above, per task and saved 50 plausible values for the latent trait variables per individual from the posterior distribution. Plausible values can be considered as imputed values for latent variables in the context of MCMC estimation [@asparouhov2010bayesian]. These plausible values were then merged for the different tasks and analyzed as multiple imputed datasets, that is, by using Rubin’s method [-@rubin1987multiple] for pooling the results from the imputations, obtaining a mean and a respective standard error for the correlation coefficient. All analyses were run in Mplus 8.10 [@muthen1998mplus] using the plausible value estimation and multiple imputation features.

### Estimation {#estimation}

Models were estimated with MPlus version 8.10 [@muthen1998mplus], using Bayesian Markov-Chain Monte-Carlo sampling, with the Mplus default priors [see simulation studies in the supplementary material of @bohn2023great]. Using inverse gamma priors `IG(0.001, 0.001)` for LST models did not substantially change the parameter estimates (see simulation study). Therefore, only the results using the MPlus default priors are reported. We used two chains with a minimum of 20,000 iterations per chain, with a thinning of 10 (corresponds to a minimum of 200,000 drawn samples per chain of which every 10th is used for the construction of the posterior distribution). The first half of each chain is discarded as burn-in. Convergence was assumed and estimation stopped when the Potential Scale Reduction (PSR) factor was well below a threshold of 1.01 for the first time after the minimum number of iterations was reached. Model syntax can be accessed by locating the respective model in the folder `outputs/` in the associated repository and opening the `.out` file using a text editor.
...
The goal of this analysis was to select the predictor variables that are relevant for predicting performance in the different cognitive tasks over time. The selection of relevant predictor variables constitutes a variable selection problem, for which a range of different methods are available (e.g., shrinkage priors). We chose to use projection predictive inference because it is a state-of-the-art variable selection procedure that provides an excellent trade-off between model complexity and accuracy [@piironen2017comparison], especially when the goal is to identify a minimal subset of predictors that yield a good predictive model [@pavone2023using].

An overview of different projection techniques and an introduction to the projection prediction approach for generalized linear models can be found in @piironen2018projective. In this work, we use the extension to the generalized linear multilevel model case provided by @catalina2022projection.  
...
The self-ordered-search task had a structurally different dependent variable, namely a count variable for each trial (0,1 or 2 redundant searches). Modeling this data would have required a poisson model. Unfortunately, we were unable to implement such a model in the projection prediction approach. Thus, we transformed the dependent variable for the self-ordered-search task so that no redundant search was coded as "correct" and one or more redundant searches were coded as incorrect. This recoding allowed us to analyse the self-ordered-searc task in the same way as all the others and makes the results comparable. However, we are aware that we loose information through this transformation.
...
```{r}
ggsave("../visuals/perf.png", height = 6, width = 12, scale = 1, bg = "white")
ggsave("../visuals/perf.pdf", height = 6, width = 12, scale = 1, bg = "white")
```
...
# latent state trait models fixed  means
...
# latent state trait models varying means
...
mot <- bind_rows(
  ...
  PPP = c(lsp$summaries$PostPred_PValue,
          ...
          lstpv$summaries$PostPred_PValue),
  `Chi 95% CI` = c(
    ...
    paste(
      lstpv$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstpv$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; ")
    
    )),
  
    tibble(
  Task = c("Probabilistic-reasoning", 
           #"",
           ""),
  Model = c("LSM",
           # "LSTM", 
  ...
  PPP = c(lsr$summaries$PostPred_PValue,
          ...
          lstrv$summaries$PostPred_PValue),
  `Chi 95% CI` = c(
    ...
    paste(
      lstrv$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstrv$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; ")
  ...
  PPP = c(lsc$summaries$PostPred_PValue,
          ...
          lstcv$summaries$PostPred_PValue),
  `Chi 95% CI` = c(
    ...
    paste(
      lstcv$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstcv$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; ")
  ...
  PPP = c(lsv$summaries$PostPred_PValue,
          ...
          lstvv$summaries$PostPred_PValue),
  `Chi 95% CI` = c(
    ...
    paste(
      lstvv$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstvv$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; ")
  ...
  PPP = c(lsi$summaries$PostPred_PValue,
          ...
          lstiv$summaries$PostPred_PValue),
  `Chi 95% CI` = c(
    ...
    paste(
      lstiv$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstiv$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; ")
  ...
  Model = c("LSM",
            #"LSTM", 
            "LSTM-V","LSTM-V (time point 7 onwards)"),
  PPP = c(lsa$summaries$PostPred_PValue,
          #lstaf$summaries$PostPred_PValue,
          lstav$summaries$PostPred_PValue,
          lstav_short$summaries$PostPred_PValue),
  `Chi 95% CI` = c(
    ...
    paste(
      lstav$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstav$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; "),
    
        paste(
      lstav_short$summaries$ObsRepChiSqDiff_95CI_LB%>%round(2)%>%format(nsmall =2),
      lstav_short$summaries$ObsRepChiSqDiff_95CI_UB%>%round(2)%>%format(nsmall =2), sep = " ; ")
...
kable_classic(full_width = F)%>%
      footnote(c("LSM = Latent state model", 
                # "LSTM = Latent state-trait model",
                 "LSTM-V = Latent state-trait model with varying means", "PPP = Posterior predictive p-value. Cut-off values (two-tailed): 0.025 and 0.975", "Chi 95% CI = 95%CI of difference between predicted and observed chi-square values"))
```
...
thresh <- bind_rows(
    ...
    #lstpf$parameters$unstandardized%>%mutate(Model = "LSTM", Task = ""),
    lstpv$parameters$unstandardized%>%mutate(Model = "LSTM-V", Task = ""),
    )%>%
  ...
  bind_rows(
    lsr$parameters$unstandardized%>%mutate(Model = "LSM", Task = "Probabilistic-reasoning"),
    #lstrf$parameters$unstandardized%>%mutate(Model = "LSTM", Task = ""),
    lstrv$parameters$unstandardized%>%mutate(Model = "LSTM-V", Task = ""),
    )%>%
    filter(paramHeader=="Thresholds")%>%
    select(Task, Model, param, est)%>%
    filter(grepl("P11\\$",param))%>%
    ...
    #lstcf$parameters$unstandardized%>%mutate(Model = "LSTM", Task = ""),
    lstcv$parameters$unstandardized%>%mutate(Model = "LSTM-V", Task = ""),
    )%>%
    filter(paramHeader=="Thresholds")%>%
    select(Task, Model, param, est)%>%
    filter(grepl("P11\\$",param))%>%
    ...
    #lstvf$parameters$unstandardized%>%mutate(Model = "LSTM", Task = ""),
    lstvv$parameters$unstandardized%>%mutate(Model = "LSTM-V", Task = ""),
    )%>%
    filter(paramHeader=="Thresholds")%>%
    select(Task, Model, param, est)%>%
    filter(grepl("P11\\$",param))%>%
  ...
  bind_rows(
    ...
    #lstif$parameters$unstandardized%>%mutate(Model = "LSTM", Task = ""),
    lstiv$parameters$unstandardized%>%mutate(Model = "LSTM-V", Task = ""),
    )%>%
    filter(paramHeader=="Thresholds")%>%
    select(Task, Model, param, est)%>%
    filter(grepl("P11\\$",param))%>%
    ...
    #lstaf$parameters$unstandardized%>%mutate(Model = "LSTM", Task = ""),
    lstav$parameters$unstandardized%>%mutate(Model = "LSTM-V", Task = ""),
    lstav_short$parameters$unstandardized%>%mutate(Model = "LSTM-V (time point 7 onwards)", Task = ""),
    )%>%
    filter(paramHeader=="Thresholds")%>%
    select(Task, Model, param, est)%>%
    filter(grepl("P11\\$",param))%>%
...
kable_classic(full_width = F)%>%
      footnote(c("LSM = Latent state model", 
                 #"LSTM = Latent state-trait model",
                 "LSTM-V = Latent state-trait model with varying means", "T1-5 = Threshold parameters for response categories"))
```
...
lstv_con_rel <- bind_rows(
  ...
  lstav_short$parameters$unstandardized%>%
  filter(grepl("New.Additional.Para",paramHeader))%>%
  select(param, est, lower_2.5ci, upper_2.5ci)%>%
    mutate(task = "attention_tp7onward"),
)%>%
  mutate(param = recode(param, 
                        "CON" = "consistency",
                        "REL" = "reliability",
...
#saveRDS(lst_overview, "../saves/lst_overview.rds")
```
...
lstv_con_rel%>%mutate(type = "varying means")
  ...
  filter(task !="attention_tp7onward")%>%
  mutate(task = recode(task,
       "pop-to-sample" = "Population-to-sample",
       "communication" = "Communicative-cues",
       "reasoning" = "Probabilistic-reasoning",
  ...
  mutate(task = factor(task, levels = c("Population-to-sample","Probabilistic-reasoning","Communicative-cues", "Ignore-visible-food", "Self-ordered-search","Attention-following" )))%>%
  filter(param != "occasion-specificity")%>%
  ...
  theme(legend.direction = "vertical", legend.position = c(0.92, 0.25), legend.box = "horizontal", legend.background = element_blank(), strip.background = element_blank(),
...
```{r lsteplot, fig.height = 3, fig.width=10, fig.cap = "Posterior mean for model parameters (with 95\\% CrI) from LSTM for the four tasks based on data.", out.width="100%"}
plste
```
...
ggsave("../visuals/ls_lst.pdf", height = 7, width = 12, scale = 1)
```
...
In the LS model, the latent state means did not change over time (see Supplementary Figure \@ref(fig:lseplot)). Reliability varied on a low to moderate level (range: `r round(range(plsr$est[plsr$task == "Population-to-sample"]),2)[1]` -- `r round(range(plsr$est[plsr$task == "Population-to-sample"]),2)[2]`). The correlations between latent states for the different time points were generally high (Supplementary Figure \@ref(fig:lsplot)). 

In the LST model, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "pop-to-sample", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "pop-to-sample", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "pop-to-sample", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This means that around 80% of true inter-individual differences are attributable to stable (trait) differences between individuals. Reliability (across time points) was estimated to be acceptable (`r round(lst_overview%>%filter(task == "pop-to-sample", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "pop-to-sample", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "pop-to-sample", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`; see Supplementary Figure \@ref(fig:lsteplot)).
...
There was no change in latent state means (see Supplementary Figure \@ref(fig:lseplot)). Reliability fluctuated on a low level (range: `r round(range(plsr$est[plsr$task == "Probabilistic-reasoning"]),2)[1]` -- `r round(range(plsr$est[plsr$task == "Probabilistic-reasoning"]),2)[2]`). The correlations between latent states for the different time points were generally low, some even negative (Supplementary Figure \@ref(fig:lsplot)). 

In the LST model, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "reasoning", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "reasoning", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "reasoning", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This means that around 80% of true inter-individual differences are attributable to stable (trait) differences between individuals. However, the reliability (across time points) was estimated to be very low, suggesting that most of the observed variance is random noise and the task is not well suited to measure inter-individual differences in ability (`r round(lst_overview%>%filter(task == "reasoning", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "reasoning", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "reasoning", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`; see Supplementary Figure \@ref(fig:lsteplot)). Thus, the consistency estimates are not meaningful in this context.

In combination with the finding that group-level performance was not reliably different from chance (see Supplementary Figure \@ref(fig:perfplot)), the low correlations between latent states and low reliability estimates suggest that this task did not measure any meaningful individual-differences in logical reasoning abilities. The task might not be suited to tap in to this particular logical reasoning ability at all. For this reason, we did not include this task when investigating the structure of individual differences and when predicting cognitive performance by external predictors. 
...
The latent state means in the LS model steadily increased over the course of the study (see Supplementary Figure \@ref(fig:lseplot)). Reliability fluctuated around a moderate level (range: `r round(range(plsr$est[plsr$task == "Communicative-cues"]),2)[1]` -- `r round(range(plsr$est[plsr$task == "Communicative-cues"]),2)[2]`). The correlations between latent states for the different time points were generally high (Supplementary Figure \@ref(fig:lsplot)), indicating realtively stable inter-individual differences in performance across time.

In the LST model with fixed means, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "communication", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "communication", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "communication", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This means that ~ 77% of true inter-individual differences are attributable to stable (trait) differences between individuals. Reliability (across time points) was estimated to be moderate to acceptable (`r round(lst_overview%>%filter(task == "communication", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "communication", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "communication", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`; see Supplementary Figure \@ref(fig:lsteplot)).

Taken together, while there was a steady increase in group-level performance (see Supplementary Figure \@ref(fig:perfplot)), individual-differences remained fairly stable which suggests an overall learning effect that was more or less the same across individuals.
...
In the LS model, the latent state means steadily increased until time point 8 and decreased again for the last two time points. Yet, the latent mean at time point 10 was still significantly different from 0 (i.e. time point 1, see Supplementary Figure \@ref(fig:lseplot)). Note, however, that the absolute level of performance was close to ceiling from time point 6 onward. Reliability fluctuated around a moderate level (range: `r round(range(plsr$est[plsr$task == "Ignore-visible-food"]),2)[1]` -- `r round(range(plsr$est[plsr$task == "Ignore-visible-food"]),2)[2]`). The correlations between latent states for the different time points were high (Supplementary Figure \@ref(fig:lsplot)), with the exception of time point 1, which did not correlate with most of the other time points.

In the LST model, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "visible-food", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "visible-food", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "visible-food", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This means that around 64% of true inter-individual differences are attributable to stable (trait) differences between individuals. Reliability (across time points) was estimated to be low to moderate (`r round(lst_overview%>%filter(task == "visible-food", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "visible-food", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "visible-food", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`; see Supplementary Figure \@ref(fig:lsteplot)).
...
From time point 6 onwards, the latent means were significantly lower compared to the first time point (i.e. subjects made fewer errors, see Supplementary Figure \@ref(fig:lseplot)). Reliability varied between moderate and good (range: `r round(range(plsr$est[plsr$task == "Self-ordered-search"]),2)[1]` -- `r round(range(plsr$est[plsr$task == "Self-ordered-search"]),2)[2]`). The correlations between latent states for the different time points were generally high (Supplementary Figure \@ref(fig:lsplot)). 

In the LST model, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "inhibit-searched", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "inhibit-searched", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "inhibit-searched", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This means that around 80% of true inter-individual differences are attributable to stable (trait) differences between individuals. Reliability (across time points) was estimated to be acceptable (`r round(lst_overview%>%filter(task == "inhibit-searched", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "inhibit-searched", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "inhibit-searched", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`;see Supplementary Figure \@ref(fig:lsteplot)).
...
Performance in the self-ordered-search task greatly benefits from using search strategies, for example, a serial strategy in which the participant starts on the left from the point of view of the experimenter (position 1), then chooses the middle (position 2) and then the right cup (position 3). In principle, any systematic search strategy is beneficial. To assess whether apes made use of such a strategy, we visualize their search patterns. 
...
search_dat_agg_pat <- read_csv("../data/inhibit_searched_data.csv")%>%
  ...
  mutate(search_pattern = ifelse(search_pattern %in% c("123", "132", "213", "231", "312", "321"), search_pattern, "redunant search"),
         serial = ifelse(search_pattern %in% c("123", "321"), "serial", "other"))%>%
  group_by(time_point, search_pattern, serial)%>%
  summarise(n = n())%>%
  group_by(time_point)%>%
  mutate(sum = sum(n))%>%
  rowwise()%>%
...
searchp <- search_dat_agg_pat%>%
  #filter(search_pattern %in% c("123", "132", "213", "231", "312", "321"))%>%
  ggplot(aes(x = factor(time_point), y = prop, col = search_pattern, lty =serial))+
  geom_point()+
  geom_line(aes(group = search_pattern))+
  labs(x = "Time point", y = "Proportion search strategy")+
  scale_color_colorblind(name = "Search pattern")+
  scale_linetype(name = "Strategy")+
  theme_bw()
```


```{r searchp, fig.height=4, fig.width=8, fig.cap = "Search strategies in the self-ordered-search task across sessions. Colors show search patterns, the orange line shows searches in which the ape made at least one redundant search. Dashed lines show serial searches.", out.width="100%"}
searchp
```

Supplementary Figure\@ref(fig:searchp) shows that, over time, apes performed less redundant searches (reflecting the overall improvement in performance). This decrease was paralleled by an increase of the two most dominant search patterns, 123 and 321. Thus, apes seem to improve in performance by developing a serial search strategy which allows them to offload memory demands. All other search strategies remained more or less constant.  
...
In the LST model, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "attention", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "attention", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "attention", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This result would usually suggest that more than 90% of true inter-individual differences are attributable to stable (trait) differences between individuals. However, the high consistency estimate contradicts the correlations between the latent states reported above, in particular the low correlations for time points 1 - 6. One potential explanation for this discrepancy is that the assumption in the LST model that the state residual variance is the same across all time points is too restrictive. If there was a switch in strategies around time point 7, it might have affected the variance in performance between apes. To account for the possibility of a chnage in strategies, we fit a second LST model only to the data from time point 7 onward. This model provided a good fit to the data (see \@ref(tab:semt)); even better compared to other models. Consistency was estimated to be high (`r round(lst_overview%>%filter(task == "attention_tp7onward", param == "consistency", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "attention_tp7onward", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "attention_tp7onward", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`) in this model, which was in line with the latent state correlation patterns. 

Reliability (across time points) was estimated to be low (`r round(lst_overview%>%filter(task == "attention", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "attention", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "attention", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`; see Supplementary Figure \@ref(fig:lsteplot)). However, when only considering the data from time point 7 and later, reliability was substantially higher (`r round(lst_overview%>%filter(task == "attention_tp7onward", param == "reliability", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "attention_tp7onward", param == "reliability", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "attention_tp7onward", param == "reliability", type == "varying means")%>%pull(upper_2.5ci),2)`)
...
cor_mod <- cor_mod_raw$parameters$stdyx.standardized%>%filter(grepl("WITH",paramHeader))%>%
  ...
  mutate(paramHeader = recode(paramHeader,
    ...
    #REASON = "XProbabilistic-reasoning",
  ...
  mutate(est = ifelse(task_1 == "Self-ordered-search", est *-1, est))%>%
  mutate(order = as.numeric(factor(task)),
         order_1 = as.numeric(factor(task_1)))

pcor_mod_a <- ggplot(cor_mod, aes(y = factor(order), x = factor(order_1), fill = est))+
  geom_tile(color = "white")+
  labs(x = "", y = "")+
  ...
  scale_y_discrete(breaks = c(1:9), labels = c(str_remove(str_remove(sort(unique(cor_mod$task)), "Z"),"X")))+
  scale_x_discrete(guide = guide_axis(angle = 90), labels = c(str_remove(str_remove(sort(unique(cor_mod$task_1)), "Z"),"X")))+
  #scale_y_discrete(limits=rev, breaks = c(1:10))+
  #scale_x_discrete(breaks = c(1:9))+
  theme_few()+
...
pcor_mod_s <- cor_mod%>%
  filter(task %in% c("ZAttention-following", "ZGaze-following", "ZCommunicative-cues"))%>%
  ggplot(., aes(y = factor(order), x = factor(order_1), fill = est))+
  geom_tile(color = "white")+
  labs(x = "", y = "")+
  scale_fill_gradient2(low = "#CC6677", high = "#117733", mid = "white", 
  midpoint = 0, limit = c(-1,1), space = "Lab", 
  ...
  #ggtitle("Model based correlations")+
  geom_text(aes(label = round(est, 2), fontface = sig), size = 3)+
  scale_y_discrete(labels = c("Att.\nfoll.","Com.\ncue."))+
  scale_x_discrete(labels = c("Com.\ncue.", "Gaze\nfoll."))+
  #scale_y_discrete(limits=rev, breaks = c(1:10))+
  #scale_x_discrete(breaks = c(1:9))+
  theme_few()+
  theme(legend.position =  c(0.15, 0.8), legend.direction = "vertical", legend.title = element_blank(), legend.background = element_blank())
...
# pcor_mod_n <- cor_mod%>%
#   filter(task %in% c("Quantity-discrimination", "Population-to-sample", "XProbabilistic-reasoning"),
#          task_1 %in% c("Quantity-discrimination", "Population-to-sample", "XProbabilistic-reasoning"))%>%
#   ggplot(., aes(y = task, x = task_1, fill = est))+
#   geom_tile(color = "white")+
#   labs(x = "", y = "")+
#   scale_fill_gradient2(low = "#CC6677", high = "#117733", mid = "white", 
#   midpoint = 0, limit = c(-1,1), space = "Lab", 
...
#   #ggtitle("Model based correlations")+
#   geom_text(aes(label = round(est, 2), fontface = sig), size = 3)+
#   scale_y_discrete(labels = c("Pop.\nsamp.","Quan.\ndisc."))+
#   scale_x_discrete(labels = c("Quan.\ndisc.","Prob.\nreas."))+
#   #scale_y_discrete(limits=rev, breaks = c(1:10))+
#   #scale_x_discrete(breaks = c(1:9))+
#   theme_few()+
#   theme(legend.position =  c(0.15, 0.8), legend.direction = "vertical", legend.title = element_blank(), legend.background = element_blank())
...
pcor_mod_e <- cor_mod%>%
  filter(task %in% c("Self-ordered-search", "Ignore-visible-food", "Delay-of-gratification"),
         task_1 %in% c("Self-ordered-search", "Ignore-visible-food", "Delay-of-gratification"))%>%
  ggplot(., aes(y = task, x = task_1, fill = est))+
  geom_tile(color = "white")+
  labs(x = "", y = "")+
  scale_fill_gradient2(low = "#CC6677", high = "#117733", mid = "white", 
  ...
  #ggtitle("Model based correlations")+
  geom_text(aes(label = round(est, 2), fontface = sig), size = 3)+
  scale_y_discrete(labels = c("Del.\ngrat.", "Ign.\nvis."))+
  scale_x_discrete(labels = c("Ign.\nvis.", "Self\nord."))+
  #scale_y_discrete(limits=rev, breaks = c(1:10))+
  #scale_x_discrete(breaks = c(1:9))+
  theme_few()+
  theme(legend.position =  c(0.15, 0.8), legend.direction = "vertical", legend.title = element_blank(), legend.background = element_blank())
...
```{r}
opcor_mod <- plot_grid(pcor_mod_a, 
          plot_grid(pcor_mod_s, pcor_mod_e,NULL, ncol = 1, labels = c("B", "C"), rel_heights = c(1,1,1.8)),
          ncol = 2, labels = c("A", ""),axis = "tb", rel_widths = c(3.5,1))
```


```{r mtmplot, fig.height=8, fig.width=10, fig.cap = "Correlations between trait estimates. Bold correlations are different from zero as judged by the 95\\% CI.", out.width="100%"}
opcor_mod
```

```{r}
ggsave("../visuals/task_level_cor.png", height = 6, width = 8, scale = 1)
ggsave("../visuals/task_level_cor.pdf", height = 6, width = 8, scale = 1)
```

For this analysis, we also included data from the tasks (excluding the switching task) reported in @bohn2023great because they tested the same individuals. We did not include the switching task from @bohn2023great because it -- similar to the probabilistic-reasoning task from the current study -- did not produce meaningful results, neither on a group nor on an individual level. The following descriptions are verbatim copies from the supplementary material of @bohn2023great.

### Tasks from Bohn et al., 2023

#### Gaze Following

The gaze following task was modeled after @brauer2005all. The experimenter sat opposite the ape and handed over food at a constant pace. That is, the experimenter picked up a piece of food, briefly held it out in front of her face and then handed it over to the participant. After a predetermined (but varying) number of food items had been handed over, the experimenter again picked up a food item, held it in front of her face and then looked up (i.e., moving her head up). The experimenter looked to the ceiling, no object of particular interest was placed there. After 10s, the experimenter looked down again, handed over the food and the trial ended. We coded whether the participant looked up during the 10s interval. Apes received eight gaze following trials. We assume that participants look up because they assume that the experimenter's attention is focused on a potentially noteworthy object.

#### Direct causal inference

The direct causal inference task was modeled after @call2004inferences. Two identical cups with a lid were placed left and right on the table. The experimenter covered the table with the occluder, retrieved a piece of food, showed it to the ape, and hid it in one the cups outside the participant's view. Next, the experimenter removed the occluder, picked up the baited cup and shook it three times, which produced a rattling sound. Next, the cup was put back in place, the sliding table pushed forwards, and the participant made a choice by pointing to one of the cups. If they picked the baited cup, their choice was coded as correct, and they received the reward. If they chose the empty cup, they did not. Participants received 12 trials. The location of the food was counterbalanced; six times in the right cup and six times in the left. Direct causal inference trials were intermixed with inference by exclusion trials (see below). We assume that apes locate the food by reasoning that the food -- a solid object -- causes the rattling sound and therefore must be in the shaken cup.
...
### Relations between trait estimates {#relations-between-traits}

Trait estimates for the tasks from @bohn2023great were computed based on the data from phase 2 because it was closer in time. Supplementary Figure\@ref(fig:mtmplot) shows the correlations between trait estimates for all tasks. Overall, most correlations were not significantly different from zero (i.e. the 95% CI did include zero). Because of the low average level of correlations, we decided not to explore models with higher-order factors and will only interpret the qualitative patterns. 

Conceptually, the tasks can be clustered in the following broader domains: *social cognition* (attention-following, gaze-following, communicative-cues), *reasoning about quantities* (quantity-discrimination, population-to-sample), *executive functions* (delay-of-gratification, self-ordered-search, ignore-visible-food) and *inferential reasoning* (probabilistic-reasoning, causal-inference, inference-by-exclusion). As a first step, we will evaluate whether we find evidence for such a clustering in the data.
...
Finally, we found a correlation between the two inferential reasoning measures. The correlation between causal-inference and inference-by-exclusion is most likely due to the fact that both tasks involve making inferences about the location of food based on reasoning about its physical properties. 
...
### Relations between learning rates {#relations-between-slopes}

In addition to analyzing the correlations between trait estimates, we analysed the correlations between learning rates. This analysis addresses the potential concern that the correlations between the trait estimates for the different tasks reflect differences in associative learning abilities, in that some individuals consistently learn faster than others which leads to individual differences in the trait estimates.

To assess correlations between learning rates, for each individual, we fit a model to the data from each task, predicting performance by time point (`brms` notation: `sum|trials(n) ~ time_point`; with `sum` being the number of correct responses per time point, `n` being the number of trials per time point and `time_point` enumerating the time point (1 to 10)). In the next step, we computed the pairwise correlations between these estimates for each combination of tasks.   

Some individuals contributed very little data for some tasks (i.e. few trials on few time points), making it difficult to estimate the corresponding slope. To exclude such cases in which the slope could not be reliably estimated, we excluded all models that did not converge (i.e. Rhat values > 2 for the slope estimate).  

```{r}
# long_bin_all <- read_csv("../data/data_binomial_models_combined.csv")
# 
...
#   dx <- ds%>%filter(id == i)
#   
#   bmij <- update(bmr, newdata = dx, chains = 3, cores = 3)
#   
...
ind_slopes <- readRDS("../saves/id_slopes.rds")
...
slope_plot <- cor_slopes%>%
  ...
  scale_y_discrete(breaks = c(1:9),labels = c(str_remove(str_remove(sort(unique(cor_slopes$task)), "Z"),"X")))+
...
```{r}
#ggsave("../visuals/cor_slopes.png", height = 8, width = 8)
```

```{r slopeplot, fig.height=8, fig.width=8, fig.cap = "Correlations between slope estimates.", out.width="75%"}
slope_plot
```

Supplementary Figure\@ref(fig:slopeplot) shows the correlations between slope estimates. In contrast to the associative learning hypothesis, which predicted strong, positive correlations across all tasks, we find a mix of positive and negative correlations with most most values being very close to zero and otherwise no clear pattern. For example, from an associative learning perspective, one would expect a strong correlation between the communicative-cues and causal-inference tasks because both involve the experimenter touching the cup that contains the food. Yet, the correlation is essentially zero. Furthermore, for the two tasks that show the strongest increase in performance over time on a group level (communicative-cues and ignore-visible-food), the individual-level slopes are negatively correlated. On the other hand, the self-ordered-search task does not involve any experimenter-given cues that could be used for learning, yet we see the strongest correlation between the slopes for this task with other tasks in which the experimenter's behavior is predictive of the reward (e.g. population-to-sample).   
...
df_trials <- data_trial%>%
  ...
  mutate(code2 = ifelse(task == "Self-ordered-search" & code > 0, 0, 1),
       code = ifelse(task == "Self-ordered-search", code2, code),
        )%>%
...
# m_att <- brm(fm,
#              family = binomial,
#              data = df_trials_final%>%filter(task == "Attention-following"),
#               warmup = 1000, 
#               iter = 4000, 
#               cores = 4, 
#               chains = 4,
...
# m_com <- brm(fm,
#              family = binomial,
#              data = df_trials_final%>%filter(task == "Communicative-cues"),
#               warmup = 1000, 
#               iter = 4000, 
#               cores = 4, 
#               chains = 4,
...
# m_inh <- brm(fm,
#              family = binomial,
#              data = df_trials_final%>%filter(task == "Self-ordered-search"),
#               warmup = 1000, 
#               iter = 4000, 
#               cores = 4, 
#               chains = 4,
...
# m_pop <- brm(fm,
#              family = binomial,
#              data = df_trials_final%>%filter(task == "Population-to-sample"),
#               warmup = 1000, 
#               iter = 4000, 
#               cores = 4, 
#               chains = 4,
...
# m_vis <- brm(fm,
#              family = binomial,
#              data = df_trials_final%>%filter(task == "Ignore-visible-food"),
#               warmup = 1000, 
#               iter = 4000, 
#               cores = 4, 
#               chains = 4,
...
#   select(-se_term)%>%
#   mutate(ranking_fulldata = factor(ifelse(ranking_fulldata == "(Intercept)","none",ranking_fulldata)),
#          select = ifelse(ranking_fulldata  %in% c("(1 | subject)", "group", "time_point", "sex"), "yes", "no"))
# 
...
ggsave("../visuals/pred_sum.pdf", height = 6, width = 12, scale = 1, bg = "white")
```
...
An alternative way to estimating the parameters of interest than using the Graded Response model for test halves in a SEM-based approach, as described above, is using a Rasch (1PL) model on the individual trial data (dichotomous responses). The Rasch model assumes that all items (trials) measure the same latent ability with the same discrimination but potentially varying difficulty. The benefit of this approach is that it makes full use of whatever data there is available for a given task, time point and subject and does not require the computation of sum scores or differentially collapsing (ordered) categories. However, we think that the parameters estimated via the SEM-based GRM approach are conceptually better aligned with the questions we are asking, in particular when it comes to reliability. In the GRM, the obtained reliability estimates quantify the amount of reliable variance in the test halves (consisting of 6 trials) when accounting for stable latent ability and true occasion-specific variance (i.e., trait and state variance). In the Rasch model, reliability quantifies the proportion of true variance in ability estimates with respect to one latent ability factor, i.e., either a state variable or a trait variable (it quantifies person separation reliability, i.e., how well true inter-individual differences can be separated from one another via the latent ability estimates). Nonetheless, we see the appeal of the Rasch model and therefore additionally present results obtained from a Rasch analysis. Overall, the two approaches yield very similar results.  
...
# 
#   state_means_rasch <- bind_rows(state_means_rasch, state_means)
# 
...
# 
#   for (j in unique(taskd$time_point)) {
# 
#     rel <-  fun_sep_rel(state, j)
# 
#     rel_tp <- rel%>%as_tibble()%>%mutate(task = i, time_point = j)
# 
#     rel_rasch <- bind_rows(rel_rasch, rel_tp)
#   }
# 
# }
# 
# saveRDS(rel_rasch, "../saves/latent_state_rel_rasch.rds")
# saveRDS(cor_rasch, "../saves/latent_state_cor_rasch.rds")
# saveRDS(state_means_rasch, "../saves/latent_state_means_rasch.rds")

rel_rasch <- readRDS("../saves/latent_state_rel_rasch.rds")
cor_rasch <- readRDS("../saves/latent_state_cor_rasch.rds")
state_means_rasch <- readRDS("../saves/latent_state_means_rasch.rds")
```

```{r}
# pairwise correlations between latent states for Rasch models

# d2 <- long%>%filter(!task %in% c("cause", "delay", "gaze", "inference", "quant"))
# 
# cor_rasch_ind_tp <- tibble()
# 
# for (j in unique(d2$task)) {
#   
# pairs <- combn(c(1:10), 2, simplify = F)
# 
# test_lsc <- d2 %>%filter(task == j)
# 
# for (i in pairs) {
#   
...
# }
# 
# saveRDS(cor_rasch_ind_tp, "../saves/cor_ls_rasch.rds")

cor_ls_rasch <- readRDS("../saves/cor_ls_rasch.rds")

```

We begin by fitting the equivalent to Latent State models to the data from each task. The `brms` formula for this model is: `code ~ 0 + time_cat + trial + (0 + time_cat | subject)`, where `code` is the response (0/1; incorrect/correct), `time_cat` is a categorical indicator of time point (1 to 10) and `trial` is a categorical indicator of trial within each time point. 

Parameterized as such, this model estimates an easiness parameter for each item (i.e. trial) as well as an easiness parameter for each time point. The latter is equivalent to a latent state mean and allows for assessing mean change over time. Note that the absolute values of this parameter are not directly comparable to the latent state means obtained in the GRM, however their pattern across time can be compared. Additionally, it is possible to compute the reliability of the latent state estimates for each time point. Here, we estimate person separation reliability in a Bayesian framework, that is, reliability for the expected a posteriori (EAP) estimate of the latent ability factor [see @adams2005reliability; @moosbrugger2007testtheorie]. In the latent state model, these refer to the reliabilities of the EAP estimates for each time-specific latent state ability factor. As noted above, this conception of reliability is different from the SEM-based approach, such that these values cannot be compared directly across methods. We also report the model-free KR20 reliability.

Finally, we estimate the correlations between latent states as an indicator of stability of individual differences across time points. Here the interpretation is the same as in the SEM-based approach. The default and recommended LKJ(1) prior for correlation matrices, as built into `brms`, is uniform over positive definite correlation matrices of a given size [see @Stan], which implies that marginal distributions of correlation parameters are increasingly concentrated over zero as the size of the correlation matrix increases. This behavior is problematic in sample sizes as small as the one in our study, which do not have the power to overrule this shrinkage. The same problem persists if we choose a different value, only that the marginal distribution then concentrates around a different value. To get around this problem, we fit pairwise models for all combinations of time points to estimate the correlations, because in such models the prior over the correlation matrix (which is of size 2x2) results in a marginal prior distribution that is uniform.

```{r}
plsm_rasch <- state_means_rasch%>%
  mutate(task = recode(task,
       "pop" = "Population-to-sample",
  ...
  mutate(task = factor(task, levels = c("Population-to-sample","Probabilistic-reasoning","Communicative-cues", "Ignore-visible-food", "Self-ordered-search","Attention-following" )))%>%
  mutate(time_point = str_remove(time_point, "time_cat"))%>%
  group_by(task)%>%
  mutate(ref = Estimate[1])%>%
  ggplot(., aes(x = time_point, y = Estimate))+
  geom_hline(aes(yintercept = ref), lty = 3, alpha = .75)+
  labs(x = "Time point",y = "Latent state mean (easiness)")+
  geom_line(aes(group = task),alpha = .75)+
  #geom_line(data = lsep%>%filter(time_point > 14), aes(group = task, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  geom_pointrange(aes(ymin = `l-95% CI`, ymax = `u-95% CI`), alpha = .75)+
  facet_grid(~task)+
  #scale_shape_manual(values = c(16,4), name = "", labels = c("includes 0", "different from 0"))+
  #geom_vline(xintercept = 14.5, size = 1)+
  #scale_x_continuous(breaks = c(1,10), labels = c(1,10))+
...
KR20_rel%>%
  mutate(task = recode(task,
       ...
       "attent" = "Attention-following"))%>%
  mutate(task = factor(task, levels = c("Population-to-sample","Probabilistic-reasoning","Communicative-cues", "Ignore-visible-food", "Self-ordered-search","Attention-following" )),
                       model = "KR20 (model-free)")%>%
   rename(value = KR20)
  
  
   # plsr%>%
   #   rename(value = est)%>%
   #   mutate(model = "SEM (MPLUS)")
   
   )%>%mutate(model = factor(model, levels = c("Rasch (brms)","KR20 (model-free)")))%>%
  ggplot(., aes(x = time_point, y = value, col = model))+
  labs(x = "Time point",y = "Reliability (person separation)")+
  geom_line(aes(group = model),alpha = .75)+
  geom_point(alpha = .75, size = 3)+
  #geom_pointrange(aes(ymin = lower_2.5ci, ymax = upper_2.5ci), alpha = .75)+
  facet_grid(~task)+
  scale_color_manual(name = "", values = c("#000000","#56B4E9"))+
  scale_x_continuous(breaks = c(1,10), labels = c(1,10))+
  scale_y_continuous(limits = c(0,1))+
  theme_few()+
  theme(legend.direction = "vertical", legend.position = c(0.075, 0.2), legend.box = "horizontal", legend.background = element_blank(), strip.background = element_blank(),
  strip.text.x = element_blank(), axis.title.x = element_blank(), axis.ticks.x = element_blank(), axis.text.x = element_blank(),
    axis.text = element_text(size = 8),       # Smaller axis text
    axis.title = element_text(size = 10),     # Smaller axis titles
    legend.text = element_text(size = 6),     # Smaller legend text
    strip.text = element_text(size = 8)      # Smaller plot title
...
```{r lseplotrasch, fig.height = 5, fig.width=10, fig.cap = "A) Latent state means estimates with 95\\% CrI for each time point estimated assuming a Rasch model for each task and time point. Dotted line denotes estimate of first time point. B) Model-based  and KR20 reliability for each time point.", out.width="100%"}
plot_grid(plsm_rasch, pls_rel, ncol = 1,labels = c("A", "B"), align = "v",axis = "l", rel_heights = c(1,1))
```


Supplementary Figure \@ref(fig:lseplotrasch) shows the latent state means and the person-separation reliability estimates for the latent state variables. The patterns of change in the latent state means are comparable to the SEM results, even though the Rasch-based estimates seem more variable. The reliability estimates are higher compared to the SEM approach, however, please note that they have a different interpretation. 

The pattern of latent state correlations in Supplementary Figure \@ref(fig:lscorplotrasch) are also similar to the ones reported above. For example, we see very high correlations in the population-to-sample task across time points, strong correlations emerging for attention-following from time point 6 onward and low or even negative correlations for probabilistic-reasoning. The overall pattern is, however, less clean for the Rasch model approach, as there are more outliers such as the high correlation between time point 1 and 2 for ignore-visible-food or a low correlation between time point 2 and 10 for communicative cues.    

```{r}
plsc_rasch <- cor_ls_rasch%>%
  mutate(task = recode(task,
       "pop" = "Population-to-sample",
       "comm" = "Communicative-cues",
       "reas" = "Probabilistic-reasoning",
...
```{r lscorplotrasch, fig.height=5, fig.width= 10, fig.cap = "Correlations between latent states estimated assuming a Rasch model for each task and time point. Correlations have been estimated via pairwise models for all possible time points.", out.width="100%"}
plsc_rasch
```

### Latent state trait models

The LST models include an additional trait variable that captures an individuals time-invariant ability. Thus, the performance at any given time point can be decomposed into a stable trait component and a variable state component. To achieve this, we added a trait variable (a constant) as part of the random intercept for subject to the model (`(0 + trait | subject)`; formula of full model: `code ~ 0 + time_cat + trial + (0 + time_cat || subject) + (0 + trait | subject)`). Thus, `trait` reflects time-invariant differences between individuals. Like in the SEM approach, we constrained the models so that the variances of the state residual variables are invariant across time (see script `constraint_lstm2.R` for how this constraint was implemented in code). 

From this model, we computed the consistency, that is, the proportion of true variance that is explained by the trait variable. This value is comparable across the two approaches and we therefore visualize them jointly. In addition, we compute the reliability of the trait (person separability for the stable trait ability factor). As noted above, this values is not directly comparable to the reliability estimates sensu SEM.  

Supplementary Figure \@ref(fig:lstmplotcomp) shows the consistency and reliability estimates for the LST model. The pattern of consistency values is comparable to the ones obtained from the SEM-based appraoch but on average slightly lower. The trait reliability values are, on the other hand, very high.   

```{r}
# run script `./analysis/constraint_lstm2.R`

# c_lst2_attent <- readRDS("../saves/constraint_lst2_attent.rds")
# c_lst2_search <- readRDS("../saves/constraint_lst2_search.rds")
# c_lst2_vfood <- readRDS("../saves/constraint_lst2_vfood.rds")
# c_lst2_reas <- readRDS("../saves/constraint_lst2_reas.rds")
# c_lst2_pop <- readRDS("../saves/constraint_lst2_pop.rds")
# c_lst2_comm <- readRDS("../saves/constraint_lst2_comm.rds")

```

```{r}
# # Consistency
# con_samp_raw_constr <- bind_rows(
# 
# con_samp_const_comm <- posterior_samples(c_lst2_comm, pars = "^sd_subject", add_chain = T)%>%
#   sample_n(5000)%>%
#   mutate(task = "comm"),
# 
# con_samp_const_attent <- posterior_samples(c_lst2_attent, pars = "^sd_subject", add_chain = T)%>%
#   sample_n(5000)%>%
#   mutate(task = "attent"),
# 
# con_samp_const_search <- posterior_samples(c_lst2_search, pars = "^sd_subject", add_chain = T)%>%
#   sample_n(5000)%>%
#   mutate(task = "search"),
# 
# con_samp_const_vfood <- posterior_samples(c_lst2_vfood, pars = "^sd_subject", add_chain = T)%>%
#   sample_n(5000)%>%
#   mutate(task = "vfood"),
# 
# con_samp_const_reas <- posterior_samples(c_lst2_reas, pars = "^sd_subject", add_chain = T)%>%
#   sample_n(5000)%>%
#   mutate(task = "reas"),
# 
# con_samp_const_pop <- posterior_samples(c_lst2_pop, pars = "^sd_subject", add_chain = T)%>%
#   sample_n(5000)%>%
#   mutate(task = "pop")
# )%>%
#   rowwise()%>%
#   mutate(con = sd_subject__trait / ( sd_subject__trait + sd_subject__time_cat1))%>%
#   ungroup()
# 
# 
# con_sum_constr <- con_samp_raw_constr%>%
#   group_by(task)%>%
...
#   
# 
# saveRDS(con_sum_constr, "../saves/consistency_constr_sample.rds")

con_sum_constr <- readRDS("../saves/consistency_constr_sample.rds")
```

```{r}
# # reliability
# rel_sum_constr <- bind_rows(
#   fun_sep_rel_LST(c_lst2_comm)%>%as_tibble()%>%mutate(task = "comm"),
#   fun_sep_rel_LST(c_lst2_attent)%>%as_tibble()%>%mutate(task = "attent"),
#   fun_sep_rel_LST(c_lst2_search)%>%as_tibble()%>%mutate(task = "search"),
#   fun_sep_rel_LST(c_lst2_vfood)%>%as_tibble()%>%mutate(task = "vfood"),
#   fun_sep_rel_LST(c_lst2_reas)%>%as_tibble()%>%mutate(task = "reas"),
#   fun_sep_rel_LST(c_lst2_comm)%>%as_tibble()%>%mutate(task = "pop")
# )
# 
# 
# rel_sum_constr <- rel_sum_constr%>%dplyr::rename(rel = value)
# 
# saveRDS(rel_sum_constr, "../saves/rel_sum_constr.rds")

rel_sum_constr <- readRDS("../saves/rel_sum_constr.rds")
...
```{r}
con_comp <- bind_rows(
lstv_con_rel%>%filter(param == "consistency", !grepl("7", task))%>%mutate(type ="SEM (MPLUS)")%>%
  select(-param)%>%
...
KR20_rel%>%
  ...
  type ="KR20 (model-free)",
    task = recode(task,
       ...
       "search" = "Self-ordered-search",
       "attent" = "Attention-following"))%>%
  group_by(task, param, type)%>%
  summarise(con_m = mean(KR20))%>%
  mutate(task = factor(task, levels = c("Population-to-sample","Probabilistic-reasoning","Communicative-cues", "Ignore-visible-food", "Self-ordered-search","Attention-following" )))

)%>%
  mutate(type = factor(type, levels = c("Rasch (brms)","SEM (MPLUS)", "KR20 (model-free)")))%>%
  ggplot(aes(x = param, y = con_m))+
  geom_pointrange(aes(ymin= con_lci, ymax = con_uci, col = type), position = position_dodge(width = 0.5))+
  theme_few()+
  ylim(0,1)+
  labs(x = "", y = "Estimate")+
  scale_color_colorblind(name = "")+
  facet_grid(~task)+
  theme(legend.direction = "vertical", legend.position = c(0.075, 0.25), legend.box = "horizontal", legend.background = element_blank()
    )
```

```{r lstmplotcomp, fig.height=4,fig.width=12, fig.cap = "Consistency estimates for the different tasks by estimation methods; reliability of trait estimates based on Rasch model and the model-free KR20 method.", out.width="100%"}
con_comp
```


```{r}
#ggsave("../visuals/con_overview.png", height = 4, width = 10, scale = 1, bg = "white")
```

### Relations between trait estimates

To estimate the correlations between the tasks, we fit pairwise LST models to all possible combinations of tasks. This pairwise approach was again chosen to get around the strong influence of the LKJ prior on the correlation matrix described above. The Rasch model approach and its implementation in `brms` forced us to have the same number of time points for all tasks. All tasks from the present study had 10 time points, however, in @bohn2023great there were 14 time points per phase. The temporal spacing was also different in the two studies. Whereas there were 8 weeks between time points in the current study, there were 2 weeks in @bohn2023great. To nevertheless have the same number of time points, we selected the last 10 time points from phase 2 in @bohn2023great and all time points for the current study. We urge the reader to keep this caveat in mind when interpreting the results. 

In the code, we added a variable denoting the task to the model, including all possible interactions with the other variables in the model. The `trait` variable in the random effects for subject was replaced with `task`; `brms` notation: `code ~ 0 + time_cat + trial + task + time_cat:task + trial:task + (0 + time_cat + time_cat:task || subject) + (0 + task | subject)`.

```{r}
# Run script `./analysis/constraint_lstm_pairwise.R`

#pairwise_cor_rasch <- readRDS("../saves/pairwise_cor_rasch.rds")

```

```{r}
# cor_mult <- pairwise_cor_rasch%>%
...
#  ggtitle("brms binomial model")+
  ...
  scale_y_discrete(breaks = c(1:9),labels = c(str_remove(str_remove(sort(unique(cor_mult$task)), "Z"),"X")))+
  scale_x_discrete(guide = guide_axis(angle = 90), labels = c(str_remove(str_remove(sort(unique(cor_mult$task_1)), "Z"),"X")))+
  theme_few()+
  theme(legend.position =  c(0.15, 0.8), legend.direction = "vertical", legend.background = element_blank())
```

```{r}
pcor_diff <- cor_mult %>%
  select(task, task_1, Estimate, order, order_1)%>%
  left_join(cor_mod%>%
              select(task, task_1, est)
            )%>%
  filter(task != task_1)%>%
  mutate(diff = Estimate - est)%>%
  ggplot(aes(y = factor(order), x = factor(order_1), fill = diff))+
  geom_tile(color = "white")+
  labs(x = "", y = "")+
  scale_fill_gradient2(high = "#4477AA", low = "#DDCC77", mid = "white", 
  midpoint = 0, limit = c(-1,1), space = "Lab", 
  name="Difference btw.\ncorrelations\n(Rasch-SEM)")+
#  ggtitle("brms binomial model")+
  ...
  scale_x_discrete(guide = guide_axis(angle = 90), labels = c(str_remove(str_remove(sort(unique(cor_mult$task_1)), "Z"),"X")))+
  theme_few()+
  theme(legend.position =  c(0.2, 0.75), legend.direction = "vertical", legend.background = element_blank())


```


```{r corbinplot, fig.height=12, fig.width=12, fig.cap = "A) Correlations between trait estimates based on Rasch model approach. Bold correlations are different from zero as judged by the 95\\% CrI. B) Correlations based on SEM approach. C) Difference in correlation coefficients (Rasch - SEM). ", out.width="100%"}
plot_grid(
  plot_grid(p_cor, pcor_mod_a, align = "h", labels = c("A", "B")), 
  plot_grid(NULL,pcor_diff, NULL, rel_widths = c(0.5,1,0.5), nrow = 1,labels = c("","C","")),
  nrow = 2)
```

Supplementary Figure \@ref(fig:corbinplot) shows the correlations between tasks based on the Rasch model approach and the SEM approach as well as the difference between the two. While the overall patter is the same (e.g., positive correlations among non-social tasks; no correlations among social tasks, positive correlation between communicative-cues and causal-inference, etc.), the Rasch approach yields slightly stronger correlations (both positive and negative).
