...
# rel_rasch <- tibble()
# 
# # Set up results matrix for reliability
# Ntp <- 10
...
```{r lseplotrasch, fig.height = 5, fig.width=8, fig.cap = "Latent means and reliability estimates with 95\\% CrI for each time point based on Rasch-model.", out.width="100%"}
plot_grid(plsm_rasch, pls_rel, ncol = 1,labels = c("A", "B"), align = "v",axis = "l", rel_heights = c(1,1))
```

There is no direct equivalent to latent state estimates in the brms approach. To assess chnage over time, we can use easisnes estimates over time ... 

Vielleicht nochmal zur Klärung: die item easiness parameter sind jetzt trial-weise (also pro "Item" aka trial) geschätzt. Für den ersten trial ist die easiness = 0. Die "easiness parameter" der Zeitpunkte sind die state means. Also in dem Sinne identisch zum Mplus Modell. Aber: die state means sind abhängig von den item easiness Werten und diese sind hier anders als die thresholds im GRM modell in Mplus, die absoluten Werte sind also nicht vergleichbar, da anders skaliert. Ihre Bedeutung ist vergleichbar (höherer Wert = höhere mittlere Fähigkeit = höhere Lösungswahrscheinlichkeit).
...
```{r}
# run script multi_lstm2.R for Rasch model 
# run script multi_lstm2_bin.R for binomial model
