...
rel_rasch%>%
  ...
  geom_line(aes(group = model, col = model),alpha = .75)+
  #geom_line(data = lsep%>%filter(time_point > 14), aes(group = task, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
...
```{r lseplotrasch, fig.height = 6, fig.width=10, fig.cap = "Latent means and reliability estimates with 95\\% CrI for each time point based on Rasch-model.", out.width="100%"}
plot_grid(plsm_rasch, pls_rel, ncol = 1,labels = c("A", "B"), align = "v",axis = "l", rel_heights = c(1,1))
```


Supplementary Figure \@ref(fig:lsplotrasch) shows ... 
...
```{r lsplotrasch, fig.height=4, fig.cap = "Correlations between latent state estimates based on the Rasch model for the different tasks.", out.width="100%"}
plsc_rasch
```

Supplementary Figure \@ref(fig:lsplotrasch) shows ... 

Values are higher when we do pairwise estimation, not all tasks together due to the strong influence of the the LKJ prior for small sample sizes 
