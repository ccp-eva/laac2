...
Supplementary Figure \@ref(fig:lstmplotcomp) shows ... 

### Relations between tasks

```{r}
# run script all_multi_lstm_bin.R
...
Pairwise correlation becasue LKJ prior underestimate the correlation (we tried that out)

* To estimate the correlations between tasks, it requires selecting the same number of time points from the two studies and treating them as if they were the same. In practice this meant that we selected the last 10 time points from the 14 time points of phase 2 of the study by @bohn2023great and re-coded them so that time point 4 was matched with time point 1 from the current study, 5 with 2 and so on. This is problematic because it neglects the temporal dynamics happening in the study prior to the 4th time point. Furthermore, the time difference between two time points was different in the two studies (2 weeks in @bohn2023great and 8 weeks in the current study).
...
plot_grid(p_cor_bin, pcro_diff, labels = c("A", "B"))
```
Supplementary Figure \@ref(fig:corbinplot) shows ... 