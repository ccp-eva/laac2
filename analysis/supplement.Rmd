...
```{r lstmplotcomp, fig.height=4,fig.width=10, fig.cap = "Consistency estimates for the different tasks by estimation methods; reliability of trait estimates based on Rasch model.", out.width="100%"}
con_comp
```
...
```{r corbinplot, fig.height=7, fig.width=14, fig.cap = "A) Correlations between trait estimates based on Rasch model approach. Bold correlations are different from zero as judged by the 95\\% CrI. B) Correlations based on SEM approach. C) Difference in correlation coefficients (Rasch - SEM). ", out.width="100%"}
plot_grid(p_cor, pcor_mod_a, pcor_diff, labels = c("A", "B", "C"), nrow = 1, align = "h")
```
