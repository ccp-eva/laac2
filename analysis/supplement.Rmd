...
time_plot_2 <- data_task%>%
  ...
  mutate(chance = ifelse(task == "Self-ordered-search", NA,ifelse(task == "Communicative-cues", 1/3,0.5)))
...
ggsave("../visuals/latent_states_reliability.pdf", width = 180, height = 80, units = "mm", scale = 1.75, bg = "white")
```
...
lstv_con_rel%>%mutate(type = "varying means")
  ...
  theme(legend.direction = "vertical", legend.position = c(0.4,0.2), legend.box = "horizontal", legend.background = element_blank(), strip.background = element_blank(),
  strip.text.y = element_blank(), axis.title.x = element_blank(), axis.ticks.x = element_blank(), axis.text.x = element_blank())
```
...
Reliability for attention is higher when only considering the later time points `r lstav_short$parameters$r2%>%filter(param == "P17")%>%pull(est)`
...
pcor_mod <- ggplot(cor_mod, aes(y = task, x = task_1, fill = est))+
  ...
  #scale_x_discrete(breaks = c(1:9))+
  theme_few()+
  theme(legend.position = c(0.2,0.75), legend.direction = "horizontal", legend.title = element_blank(), legend.background = element_blank())
```
...
pcpro <- ggplot(proj_samples_com, aes(x = value, y = pred)) +
...
pvissel <- vis_varselp%>%
  ...
  theme_few()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = c(0.5,0.6), legend.direction = "horizontal")
...
ggplot(proj_samples_sum, aes(x = mean, y = pred, col = task))+
