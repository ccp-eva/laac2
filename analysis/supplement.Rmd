...
```{r lseplotrasch, fig.height = 6, fig.width=10, fig.cap = "Latent state means and trait reliability estimates with 95\\% CrI for each time point estimated assuming a Rasch model for each task and time point.", out.width="100%"}
plot_grid(plsm_rasch, pls_rel, ncol = 1,labels = c("A", "B"), align = "v",axis = "l", rel_heights = c(1,1))
```


Supplementary Figure \@ref(fig:lseplotrasch) shows the latent state means and the trait reliabilities. The temporal dynamics in the latent trait estimates are comparable to the SEM results, even though the Rasch-based estimates seem more variable. The reliability estimates are higher compared to the SEM approach, however, please note that they have a different interpretation because the refer to the separability of individuals (trait reliability) and not the reliability of the measurement instrument. 

The pattern of latent state correlations in Supplementary Figure \@ref(fig:lscorplotrasch) are also similar to the ones reported above for the SEM approach reported above. For example, we see very high correlations in the population-to-sample task across time points, strong correlations emerging for attention-following from time point 6 onward and low or even negative correlations for probabilistic-reasoning. The overall pattern is, however, less clean for the Rasch approach, as there are outliers such as the high correlation between timep point 1 and 2 for ignore-visible-food or a low correlation between time point 2 and 10 for communicative cues.    
...
```{r lstmplotcomp, fig.height=4,fig.width=10, fig.cap = "Consistency estiamtes for the different tasks by estimation methods.", out.width="100%"}
con_comp
```
...
```{r corbinplot, fig.height=7, fig.width=14, fig.cap = "A) Correlations between trait estimates based on a binomial model. Bold correlations are different from zero as judged by the 95\\% CrI. B) Difference in correlation coefficients (SEM - binomial). ", out.width="100%"}
plot_grid(p_cor, pcor_diff, labels = c("A", "B"))
```
