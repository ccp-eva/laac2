...
This document supplements the paper "The structure and predictability of individual differences in great ape cognition across domains". Some text passages and figures are the same in the supplementary material and the main paper. This redundancy is intended and ensures that the supplementary material is self-contained and readable. 
...
Apes were tested in familiar sleeping or observation rooms by a single experimenter. Whenever possible, they were tested individually. It is also noted down, whenever other mebers of the group observed the individual during sessions. The basic setup comprised a sliding table positioned in front of a mesh. Sometimes the mesh has been exchanged for a clear plexiglas panel with three holes in it (due to logistical reasons). The experimenter sat on a small stool and used an occluder to cover the sliding table (see Supplementary Figure \@ref(fig:setup)).
...
The Attention-following task was loosely modeled after @kaminski2004body. The setup consisted of two identical cups placed on the sliding table and a large opaque screen that was longer than the width of the sliding table (Supplementary Figure \@ref(fig:setup)F). The experimenter placed both cups on the table and showed the ape that they were empty. Then, the experimenter baited both cups in view of the ape and placed the opaque screen in the center between the two cups, perpendicular to the mesh. Next, the experimenter moved to one side and looked at the cup in front of them. Then, the experimenter pushed the sliding table forward and the ape was allowed to choose one of the cups by pointing at it through the mesh. If the ape chose the cup that the experimenter was looking at, they received the food item. If they choose the other cup, they did not. Because the experimenter could not see if the ape chose the other cup, the trial was terminated if the ape was at the mesh (in a position to make a choice) and did not choose the cup the experimenter was looking at within 2s. Whether or not the ape received the food, the experimenter then neutrally and calmly clears the table (screen and cups). Ideally in such a way that the ape did not see the food, but also not in a way that would give it the impression that the experimenter is intentionally hiding the food. Then, the next trial begins. We coded whether the ape chose the side the experimenter was looking at (correct choice) or not. Apes received twelve trials. The side at which the experimenter looked was counterbalanced with same number of looks to each side and looks to the same side not more than two times in a row.
...
The task was modeled after @hanus2014maths. Three identical cups were distributed on top of two plastic trays (30 cm x 20 cm) presented side-by-side on a sliding table (Supplementary Figure \@ref(fig:setup)B). Two half-open boxes served as occluders to block the ape’s view when shuffling the cups. Each trial started by showing the ape that all three cups (one on one tray, two on the other) were empty. After placing the occluders over the trays, the experimenter put one piece of food on top of each occluder. Next, the experimenter hid each piece of food under the cup(s) behind the occluders. In case of the occluder with the two cups, the food was randomly placed under one of the two cups while both cups were visited. Finally, both occluders were lifted and the table pushed forwards, allowing the ape to choose one cup from either tray, from which they then received the content. We coded whether the ape chose the certain cup (i.e. the cup from the tray with only one cup). Apes received 12 trials. The side with the tray with one cup was counterbalanced, with the same constellation appearing not more than two times in a row on the same side.
...
The task was modeled after @volter2019chimpanzees [see also @petrides1995impairments; @diamond1997prefrontal]. Three identical cups were placed equidistantly on a sliding table directly in front of the ape (Supplementary Figure \@ref(fig:setup)E). The experimenter baited all three cups in full view of the ape. Next, the experimenter pushed the sliding table forwards for the ape to choose one of the cups by pointing. After the choice, the table was pulled back and the ape received the food. After a 2s pause, the table was pushed forward again for a second choice. This procedure was repeated for a third choice. If the ape chose a baited cup, they received the food, if not, not. We coded the number of times the ape chose and empty cup (i.e. chose a cup they already chose before). Please note that this outcome variable differed from the other tasks in two ways: first, possible values were 0, 1, and 2 (instead of just 0 and 1) and second, a lower score indicated better abilities. Apes received twelve trials. No counterbalancing was needed.
