...
In the LST model, the consistency coefficient was estimated to be `r round(lst_overview%>%filter(task == "attention", param == "consistency", type == "varying means")%>%pull(value),2)` (95% CI: `r round(lst_overview%>%filter(task == "attention", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "attention", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`). This result would usually suggest that more than 90% of true inter-individual differences are attributable to stable (trait) differences between individuals. However, this result contradicts the correlations between the latent states reported above, in particular the low correlations for time points 1 - 6. One potential explanation for this discrepancy is that the assumption in the LST model that the state residual variance is the same across all time points is too restrictive. If there was a switch in strategies around time point 7, this might have affected the variance in performance between apes. To account for this possibility, we fit a second LST model only to the data from time point 7 onward. This model provided a good fit to the data (see \@ref(tab:semt)); even better compared to other models. Consistency was estimated to be high (`r round(lst_overview%>%filter(task == "attention_tp7onward", param == "consistency", type == "varying means")%>%pull(value),2)`; 95% CI: `r round(lst_overview%>%filter(task == "attention_tp7onward", param == "consistency", type == "varying means")%>%pull(lower_2.5ci),2)` - `r round(lst_overview%>%filter(task == "attention_tp7onward", param == "consistency", type == "varying means")%>%pull(upper_2.5ci),2)`) in this model, which was in line with the latent sate correlation patterns. 
...
pcor_mod_a <- ggplot(cor_mod, aes(y = factor(order), x = factor(order_1), fill = est))+
  ...
  scale_y_discrete(breaks = c(1:10), labels = c(str_remove(str_remove(sort(unique(cor_mod$task)), "Z"),"X")))+
...
opcor_mod <- plot_grid(pcor_mod_a, 
          plot_grid(pcor_mod_s, pcor_mod_n,pcor_mod_e,NULL, ncol = 1, labels = c("B", "C", "D")),
