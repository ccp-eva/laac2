...
d1 <- long_bin%>%filter(task %in% c("cause", "delay", "gaze", "inference", "quant"))

Multi_LST1_bin <-  brm(sum|trials(n) ~ 0 + time_cat + task + time_cat:task + 
...
saveRDS(Multi_LST1_bin, "../saves/multi_lst1_bin.rds")
