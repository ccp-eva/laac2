...
df_trials_final_inh <- df_trials_inh %>%
  ungroup()%>%
  mutate(across(c(sick_severity,
                  test_tp,
                  rel_rank,
  ...
  left_join(read_csv("../saves/srm_estimates.csv")%>%
              ...
              select(subject, time_point, mean)%>%
              dplyr::rename("sociality" = "mean"))%>%
  left_join(read_csv("../data/LAAC_inhibit_searched_long_format.csv")%>%
  select(date, time_point, subject, trial, error.ny))%>%
  mutate(ot = log(1/3), 
         tp_fac = factor(time_point))
...
m_inh_long <- brm(error.ny ~ offset(ot)+
                sick_severity +
                test_tp +
                test_day +
                rel_rank +
...
# delay random intercept to last place so that it doesn't soak up all the variance
s_terms_inh <- c("1", all_fixed_effects,
             paste0(paste(all_fixed_effects, collapse = " + "),
                " + (1 | subject)"),
                  paste0(paste(all_fixed_effects, collapse = " + "), 
                          "+ (time_point|subject)"))
...
cvs_inh_long <- cv_varsel(m_inh,
                     search_terms = s_terms_inh,
                     validate_search = T,
                     cv_method = "LOO",
                     method = "forward",
                     parallel = TRUE,
...
saveRDS(cvs_inh_long,"../saves/cvs_inh_long.rds")
...
```{r}
plot(cvs_vis, stats = "elpd", ranking_nterms_max = NA)
plot(cvs_vis, stats = "elpd", deltas = TRUE)

smmry <- summary(cvs_com, stats = "elpd", type = c("mean", "se", "diff", "diff.se"),
                 deltas = T)
