...
df_trials_final <- df_trials %>%
  ...
  left_join(read_csv("../saves/srm_estimates.csv")%>%select(subject, time_point, mean)%>%dplyr::rename("sociality" = "mean"))
...
df_trials_final <- df_trials_final %>%
  ...
  mutate(
    rel_rank = case_when(
      ...
      group == "bonobos" ~ percent_rank(grp_size$bonobos:1)[rank],
      group == "gorillas" ~ percent_rank(grp_size$gorillas:1)[rank],
      group == "orangs" ~ percent_rank(grp_size$orangs:1)[rank]
    )
  ) %>%
  ungroup()
